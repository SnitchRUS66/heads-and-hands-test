...
export default {
  ...
  methods: {
    init() {
      ...
      if (
        ...
        (!this.initWithoutMarkers &&
          !this.$slots.default &&
          !this.placemarks.length)
      )
        return;
      ...
      for (let i = 0; i < myMarkers.length; i++) {
        const m = myMarkers[i];
        const markerType = utils.createMarkerType(
          m.markerType,
        ...
        const strokeOptions = m.markerStroke
          ? {
              ...
              strokeOpacity:
                parseFloat(m.markerStroke.opacity) >= 0
                  ? parseFloat(m.markerStroke.opacity)
                  : 1,
              strokeStyle: m.markerStroke.style,
              strokeWidth:
                parseFloat(m.markerStroke.width) >= 0
                  ? parseFloat(m.markerStroke.width)
                  : 1
            }
          : {};
        ...
        const fillOptions = m.markerFill
          ? {
              ...
              fillOpacity:
                parseFloat(m.markerFill.opacity) >= 0
                  ? parseFloat(m.markerFill.opacity)
                  : 1,
      ...
      if (this.zoomControl) {
        ...
        this.myMap.controls.add(
          new ymaps.control.ZoomControl(this.zoomControl)
        );
      }
      if (this.detailedControls) {
        ...
        controls.forEach(controlName => {
          ...
          this.myMap.controls.add(
            controlName,
            this.detailedControls[controlName]
          );
        });
      }
  ...
  watch: {
    coordinates(newVal) {
      // TODO: ULTRA HACK - MUST BE REWORKED
  ...
  mounted() {
    ...
    // Setup the observer
    this.observer.observe(document.querySelector(".ymap-markers"), {
      attributes: true,
      childList: true,
      characterData: true,
      subtree: true
    ...
    if (this.ymapEventBus.scriptIsNotAttached) {
      ...
      const mapLink =
        this.mapLink || "https://api-maps.yandex.ru/2.1/?lang=ru_RU";
